...
let RecommendationService = exports.RecommendationService = (_dec = (0, _common.Injectable)(), _dec2 = (0, _common.Dependencies)('RecommendationRepository', 'ItemRepository'), _dec3 = function (target, key) {
  return (0, _typeorm.InjectRepository)(_recommendationEntity.Recommendation)(target, undefined, 0);
}, _dec4 = function (target, key) {
  return (0, _typeorm.InjectRepository)(_itemEntity.Item)(target, undefined, 1);
}, _dec5 = Reflect.metadata("design:type", Function), _dec6 = Reflect.metadata("design:paramtypes", [void 0, void 0]), _dec(_class = _dec2(_class = _dec3(_class = _dec4(_class = _dec5(_class = _dec6(_class = class RecommendationService {
  constructor(recommendationRepository, itemRepository) {
  ...
  async generateRecommendations(userId, preferences = {}) {
    try {
      ...
      const allRecommendations = this.generateOutfitCombinations(itemsByCategory, preferences.occasion, preferences.weather);
      ...
      for (const rec of limitedRecommendations) {
        try {
          ...
          const itemEntities = await this.itemRepository.findByIds(rec.items.map(item => item.id));
  ...
  async getRecommendations(userId, filters = {}) {
    const query = this.recommendationRepository.createQueryBuilder('r').leftJoin('r.items', 'i').where('r.userId = :userId', {
      userId
    }).andWhere('r.rejectedAt IS NULL').select(['r.id', 'r.title', 'r.occasion', 'r.wasWorn', 'r.createdAt', 'i.id', 'i.name', 'i.category', 'i.imageUrl']).orderBy('r.createdAt', 'DESC');
    if (filters.occasion) {
      query.andWhere('r.occasion = :occasion', {
  ...
  async markAsWorn(userId, recommendationId) {
    ...
    if (!recommendation) {
      throw new _common.NotFoundException('Recommendation not found');
    }
  ...
  deduplicateRecommendations(recommendations) {
    ...
    for (const rec of recommendations) {
      const combinationKey = rec.items.map(item => item.id).sort((a, b) => a - b).join('-');
