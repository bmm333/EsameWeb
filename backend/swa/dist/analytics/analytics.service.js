...
let AnalyticsService = exports.AnalyticsService = (_dec = (0, _common.Injectable)(), _dec2 = (0, _common.Dependencies)('ItemRepository', 'OutfitRepository'), _dec3 = function (target, key) {
...
}, _dec5 = Reflect.metadata("design:type", Function), _dec6 = Reflect.metadata("design:paramtypes", [void 0, void 0]), _dec(_class = _dec2(_class = _dec3(_class = _dec4(_class = _dec5(_class = _dec6(_class = class AnalyticsService {
  ...
  async getBasicWardrobeStats(userId) {
    try {
      const [totalItems, totalOutfits, favoriteItems] = await Promise.all([this.itemRepository.count({
        where: {
          userId
        }
  ...
  async getWardrobeAnalytics(userId) {
    try {
      const [mostWornItems, categoryStats, colorAnalysis, sustainabilityStats, rarelyUsedItems] = await Promise.all([this.getMostWornItems(userId), this.getCategoryBreakdown(userId), this.getColorAnalysis(userId), this.getSustainabilityStats(userId), this.getRarelyUsedItems(userId)]);
      return {
        mostWornItems,
        categoryStats,
        colorAnalysis,
  ...
  async getCategoryBreakdown(userId) {
    try {
      const result = await this.itemRepository.createQueryBuilder('item').select('item.category', 'category').addSelect('COUNT(*)', 'count').addSelect('SUM(item.wearCount)', 'totalWears').addSelect('AVG(item.wearCount)', 'avgWears').where('item.userId = :userId', {
        userId
      }).groupBy('item.category').getRawMany();
  ...
  async getRarelyUsedItems(userId) {
    try {
      ...
      return rarelyUsedItems.filter(item => (item.wearCount || 0) < 2).map(item => {
        let monthsOld = 1;
        if (item.dateAdded) {
          const now = new Date();
  ...
  async getColorAnalysis(userId) {
    try {
      ...
      const mostWornColors = Object.values(colorStats).sort((a, b) => b.totalWears - a.totalWears).slice(0, 10);
      return {
  ...
  async getSustainabilityStats(userId) {
    try {
      ...
      if (avgWearsPerItem >= 10) sustainabilityScore = 90;else if (avgWearsPerItem >= 7) sustainabilityScore = 80;else if (avgWearsPerItem >= 5) sustainabilityScore = 70;else if (avgWearsPerItem >= 3) sustainabilityScore = 60;else if (avgWearsPerItem >= 1) sustainabilityScore = 40;else sustainabilityScore = 20;
  ...
  async getActionableInsights(userId) {
    try {
      const [basicStats, rarelyUsedItems, sustainabilityStats] = await Promise.all([this.getBasicWardrobeStats(userId), this.getRarelyUsedItems(userId), this.getSustainabilityStats(userId)]);
      const insights = [];
