...
let RfidService = exports.RfidService = (_dec = (0, _common.Injectable)(), _dec2 = function (target, key) {
  return (0, _typeorm.InjectRepository)(_rfidDeviceEntity.RfidDevice)(target, undefined, 0);
}, _dec3 = function (target, key) {
  return (0, _typeorm.InjectRepository)(_rfidTagEntity.RfidTag)(target, undefined, 1);
}, _dec4 = function (target, key) {
  return (0, _typeorm.InjectRepository)(_itemEntity.Item)(target, undefined, 2);
}, _dec5 = function (target, key) {
  return (0, _typeorm.InjectDataSource)()(target, undefined, 3);
}, _dec6 = Reflect.metadata("design:type", Function), _dec7 = Reflect.metadata("design:paramtypes", [void 0, void 0, void 0, void 0]), _dec(_class = _dec2(_class = _dec3(_class = _dec4(_class = _dec5(_class = _dec6(_class = _dec7(_class = class RfidService {
  constructor(deviceRepo, tagRepo, itemRepo, dataSource) {
  ...
  async validateApiKey(apiKey) {
    ...
    if (!device) throw new _common.UnauthorizedException('Invalid API key');
    return device;
  }
  ...
  async associateTag(userId, tagId, itemId, forceOverride = false) {
    ...
    return await this.dataSource.transaction(async manager => {
      const item = await manager.findOne(_itemEntity.Item, {
        where: {
          id: itemId,
          userId
        },
      ...
      if (!item) {
        ...
        throw new _common.NotFoundException(`Item with ID ${itemId} not found or doesn't belong to user`);
      }
      let tag = await manager.findOne(_rfidTagEntity.RfidTag, {
        where: {
          tagId,
          userId
        },
      ...
      if (!tag) {
        ...
        if (!device) throw new _common.NotFoundException('No RFID device found for user');
        tag = manager.create(_rfidTagEntity.RfidTag, {
          tagId,
          userId,
          deviceId: device.id,
          status: 'detected',
      ...
      if (tag.itemId && tag.itemId !== itemId) {
        existingItem = await manager.findOne(_itemEntity.Item, {
          where: {
            id: tag.itemId,
            userId
          },
      ...
      const finalItem = await manager.findOne(_itemEntity.Item, {
        where: {
          id: itemId,
          userId
        },
      ...
      if (!finalItem) {
        throw new _common.NotFoundException('Target item was deleted during operation');
      }
      tag.itemId = itemId;
      tag.location = 'wardrobe';
      await manager.save(tag);
